...
public:
	...
	* @param endPos 机器人末端的位姿4*4矩阵,单位：m + rad
	...
	*/
	...
	/**
	 * @brief 在本地由当前6轴角度计算逆运动学
	 * @param joints_ref 当前机器人的关节角(6x1),单位：deg
	 * @param end 机械臂末端的位姿(6x1):XYZ+欧拉角,单位：mm + deg
	 * @param joints_out 机器人的关节角(6x1),单位：deg
	 * @return 
	*/
	void Cal_iKine(double* joints_ref, double* end, double* joints_out);

	/**
	 * @brief 先运动到色块正上方,然后垂直向下运动抓住色块,再 根据信息 提起色块一定高度
	 * @param blob 
	 * @param height_offset 由已抓取色块个数决定的高度偏置
	 * @param speed 关节速度(deg/s)
	*/
	void Crab_Blob(Blob& blob, double height_offset, double speed);

	/**
	 * @brief 通过CvMove将色块运输到指定位置
	 * @param blob 
	 * @param height_offset 由已抓取色块个数决定的高度偏置
	 * @param cnt 轨迹上离散点的总数
	*/
...
public:
	...
	int blob_num[6] = {0,0,0,0,0,0}; //存每种颜色的色块的个数
private:
